...
<body>
<?php
echo "<h2><hr>Requête qui affiche la liste des personnes dont le prénom commence par la lettre T dans la table etudiants</h2>";
...
try {
  ...
  $stmt = $conn->prepare("SELECT $requete FROM $table WHERE $critere_type LIKE '$critere_valeur'");
  $stmt->execute();
